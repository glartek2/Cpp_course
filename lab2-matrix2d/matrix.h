...
public:
     // Constructors
    TwoDimensionMatrix(); // Default constructor
    TwoDimensionMatrix(const TwoDimensionMatrix& other); // Copy constructor
    TwoDimensionMatrix(const MatrixElement matrix[size_][size_]); // Constructor taking 2x2 array
    ...
    // Accessor function to get an element
    MatrixElement get(size_t row, size_t col) const;

    // Static member function to return size
    ...
    TwoDimensionMatrix& operator*=(MatrixElement number) {
        for (size_t i = 0; i < size_; ++i) {
            for (size_t j = 0; j < size_; ++j) {
                MatrixElement& element = matrix_[i][j];
                element *= number;
            }
        }
        return *this;
    ...
    TwoDimensionMatrix operator&&(const TwoDimensionMatrix& matrix) const {
    MatrixElement resultArray[TwoDimensionMatrix::size_][TwoDimensionMatrix::size_];
    for (size_t i = 0; i < size_; ++i) {
        for (size_t j = 0; j < size_; ++j) {
            // Perform logical AND operation on corresponding elements
            resultArray[i][j] = get(i, j) && matrix.get(i, j);
        }
    }
